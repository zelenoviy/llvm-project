...
void haiku::Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // When building 32-bit code on Haiku/amd64, we have to explicitly
...
llvm::ExceptionHandling Haiku::GetExceptionModel(const ArgList &Args) const {
  // Haiku uses SjLj exceptions on ARM oabi.
...
bool Haiku::isPIEDefault(const llvm::opt::ArgList &Args) const {
